...
「ログを集めて分析基盤を作ったけど、誰も見ていない」——これは SIEM あるあるで、業界では **SIEM シェルフウェア**と呼ばれる。高価なツールを導入したのに棚に飾られたまま、という意味だ。
...
| | AWS ネイティブ構成 | フル SIEM |
|---|---|---|
| 検知ルール | AWS が書いて管理 | 自分たちで書いてチューニング |
| 運用コスト | 低い | 高い（専任が必要） |
| カスタム検知 | できない | できる |
| 対応できる脅威 | AWS が想定する脅威 | 自社固有の脅威も含めて定義可能 |

SIEM を本格的に建てるのは「GuardDuty が出してくれる以上のカスタム検知が業務上必要」「専任のセキュリティエンジニアがいる」という段階に達してから、が現実解。それ以前に SIEM を建てると、ほぼ確実にシェルフウェアになる。
