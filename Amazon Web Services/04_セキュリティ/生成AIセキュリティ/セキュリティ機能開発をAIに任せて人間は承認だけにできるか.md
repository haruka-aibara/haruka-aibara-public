...
- **AI の挙動自体を監視対象にする**。AI 実行ロールの CloudTrail イベントは通常の開発者と別のベースラインで見る。GuardDuty・Security Hub CSPM のアラート対象に「AI が想定外の API を呼んだ」を含める
...
- 検知ルールは**攻撃シミュレーションとセットで納品させる**。GuardDuty のサンプル検出、Security Hub CSPM へのテストイベント投入、Atomic Red Team 的な安全な攻撃再現をサンドボックスで実行し、「検知した証拠」を証拠パッケージに含める
