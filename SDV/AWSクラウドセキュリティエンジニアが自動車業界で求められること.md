...
OEM が UN R155 の型式認証を取るには、CSMS（サイバーセキュリティ管理システム）の組織認証が必要だ。そのバックエンドとして「クラウド上での脆弱性監視・インシデント対応・監査ログ管理」の仕組みを作る案件が出てくる。Security Hub CSPM + GuardDuty + CloudTrail の組み合わせが基盤になる。
