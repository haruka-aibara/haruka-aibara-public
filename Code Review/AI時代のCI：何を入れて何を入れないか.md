...
tfsec と Checkov は重複する部分があるが、補完的に機能する。両方いれるなら soft_fail の閾値を揃えておかないと CI の挙動が混乱する。

### コンテナスキャン（Dockerfile 変更時のみ）
...
| Terraform セキュリティ | tfsec + Checkov | L3（Terraform 変更時）|
| コンテナ脆弱性 | trivy | L3（Dockerfile 変更時）|
