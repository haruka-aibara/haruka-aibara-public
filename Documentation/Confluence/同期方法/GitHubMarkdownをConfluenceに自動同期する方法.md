...
mark は `<!-- Parent: xxx -->` で指定した親ページが存在しない場合、自動で作成する。ただし、その親ページ自体がどこに属すべきかは解決できないため、スペースのルート直下に作られてしまう。

例: `セットアップ/セットアップ.md`（`Parent: Confluence`）より先に `セットアップ/記事.md`（`Parent: セットアップ`）が処理された場合、`セットアップ` が `Confluence` の下ではなくルート直下に作成される。

→ **対策**: ファイルをパスの深さ順に処理し、インデックスページ（フォルダ名と同名の .md）を同階層内で優先する。このリポジトリのワークフローはこの順序で処理している。

**削除機能はない**

mark にはファイル削除に連動して Confluence ページを削除する機能はない（README で確認済み）。ページの削除は Confluence 上で手動対応が必要。エンタープライズ用途では自動削除は事故リスクがあるため、手動運用が適切。

**docker run 時のコマンド指定**

mark の Docker イメージは `dumb-init` がエントリーポイントのため、`docker run` では `mark` コマンド名を明示する必要がある。フラグは `-f`（`-file` / `--file` は存在しない）。

```bash
docker run ... kovetskiy/mark:v15.3.0 mark -f path/to/file.md
```
